...
One-vs-Rest's real practical advantage: it's trivially parallelizable (every classifier trains completely independently, no coordination needed) and lets you reuse ANY binary classifier as a building block, not just logistic regression, `07-lda`'s Linear Discriminant Analysis, `03-classical-ml/05-instance-based-probabilistic`'s Naive Bayes and k-NN, or even a Support Vector Machine, none of which have a native multiclass formulation the way softmax does, can all be turned multiclass via exactly this same One-vs-Rest wrapper.
