...
output_projection = load_solution("07-transformers-llm/03-language-model-assembly/01-output-projection").output_projection
linear_backward = load_solution("05-dl-training/02-layers/02-linear-backward").linear_backward
_losses = load_solution("04-deep-learning-core/03-losses/02-cross-entropy")
...
def train_output_head_one_step(
    ...
    (mirroring `[05-dl-training/03-training-loop/02-assemble-training-loop]`'s
    ...
    `[05-dl-training/02-layers/02-linear-backward]`'s `linear_backward`),
