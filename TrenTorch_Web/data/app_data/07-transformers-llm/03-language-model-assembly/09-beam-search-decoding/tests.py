"""
pytest data/app_data/07-transformers-llm/03-language-model-assembly/09-beam-search-decoding/tests.py
"""
...
_module = load_solution(
    f"07-transformers-llm/03-language-model-assembly/{Path(__file__).resolve().parent.name}"
)
...
def test_sequence_log_prob_is_negative_of_what_next_token_loss_would_sum_to():
    ...
    next_token_cross_entropy_loss = load_solution(
        "07-transformers-llm/03-language-model-assembly/03-next-token-cross-entropy"
    ...
    full_lm_forward = load_solution("07-transformers-llm/03-language-model-assembly/04-full-forward-pass").full_lm_forward
