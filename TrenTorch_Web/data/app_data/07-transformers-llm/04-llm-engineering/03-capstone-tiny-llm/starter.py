...
char_tokenize = load_solution("06-seq-modeling/01-tokenization/01-whitespace-char-tokenizer").char_tokenize
_vocab_module = load_solution("06-seq-modeling/01-tokenization/02-vocabulary-building")
build_vocabulary = _vocab_module.build_vocabulary
encode_with_unk = _vocab_module.encode_with_unk
build_inverse_vocab = load_solution("06-seq-modeling/01-tokenization/05-encode-decode-roundtrip").build_inverse_vocab

embedding_forward = load_solution("06-seq-modeling/02-embeddings/01-token-embedding-lookup").embedding_forward
sinusoidal_positional_encoding = load_solution(
    "06-seq-modeling/02-embeddings/03-sinusoidal-positional-encoding"
).sinusoidal_positional_encoding
combine_embeddings = load_solution("06-seq-modeling/02-embeddings/05-combine-token-positional-embeddings").combine_embeddings

stack_transformer_blocks = load_solution("07-transformers-llm/01-transformer-block/07-stack-blocks").stack_transformer_blocks
build_causal_mask = load_solution("06-seq-modeling/04-attention/02-causal-mask").build_causal_mask
train_output_head = load_solution("07-transformers-llm/03-language-model-assembly/05-training-loop").train_output_head
greedy_decode = load_solution("07-transformers-llm/03-language-model-assembly/07-greedy-decoding").greedy_decode


def build_char_vocab(corpus_text: str) -> dict[str, int]:
    """
    `[06-seq-modeling/01-tokenization/01-whitespace-char-tokenizer]`'s
