...
Because NumPy arrays are mutable, passing the exact same `embedding_table` array into BOTH `[06-seq-modeling/02-embeddings/01-token-embedding-lookup]`'s lookup AND this question's `output_projection_tied` means any later modification to that array (e.g. a gradient update during training) is automatically visible from both call sites, no extra syncing code required. That's the entire mechanism weight tying relies on.
