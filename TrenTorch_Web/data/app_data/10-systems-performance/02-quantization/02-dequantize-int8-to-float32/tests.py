"""
pytest data/app_data/10-systems-performance/02-quantization/02-dequantize-int8-to-float32/tests.py
"""
...
_module = load_solution(f"10-systems-performance/02-quantization/{Path(__file__).resolve().parent.name}")
dequantize = _module.dequantize
quantization_error = _module.quantization_error
quantize = load_solution("10-systems-performance/02-quantization/01-quantize-float32-to-int8").quantize
