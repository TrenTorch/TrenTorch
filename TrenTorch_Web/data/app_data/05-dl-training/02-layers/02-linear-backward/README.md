...
`y = x @ weight.T`, so by the matmul backward rule (`[04-deep-learning-core/04-autograd/03-backward-matmul]`'s `matmul_backward`, with `a = x` and `b = weight.T`), `grad_x = grad_output @ weight` (since `(weight.T).T = weight`).
