"""
pytest data/app_data/06-seq-modeling/04-attention/05-mha-concat-output-projection/tests.py
"""
...
_module = load_solution(f"06-seq-modeling/04-attention/{Path(__file__).resolve().parent.name}")
concat_heads = _module.concat_heads
multi_head_attention = _module.multi_head_attention

split_heads = load_solution("06-seq-modeling/04-attention/04-mha-split-heads").split_heads
...
def test_identity_projection_leaves_concatenated_output_unchanged():
    ...
    per_head_output, _ = load_solution(
        "06-seq-modeling/04-attention/04-mha-split-heads"
