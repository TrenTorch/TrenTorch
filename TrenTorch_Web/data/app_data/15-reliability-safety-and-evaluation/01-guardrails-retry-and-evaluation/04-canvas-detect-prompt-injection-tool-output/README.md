...
The chain runs `Tool output received` -> `Scan for injection patterns`, and only from the scan does the flow branch: clean output proceeds to `Pass to model, tagged as data` (note: tagged as _data_, never merged into the model's own instructions), while suspicious output goes to `Sanitize and flag` instead. Neither distractor piece connects to anything, since both describe the exact failure this guardrail exists to prevent — executing an embedded instruction, or trusting the tool's output at the same level as a real system instruction.
