"""
pytest data/app_data/09-vision/03-cnn-architecture/02-one-cnn-block/tests.py
"""
...
cnn_block = load_solution(f"09-vision/03-cnn-architecture/{Path(__file__).resolve().parent.name}").cnn_block
conv2d_multi_filter = load_solution(
    "09-vision/01-convolutions/05-multiple-output-filters"
).conv2d_multi_filter
relu_forward = load_solution("04-deep-learning-core/02-activations/01-relu").relu_forward
max_pool2d = load_solution("09-vision/02-pooling/01-max-pooling").max_pool2d
