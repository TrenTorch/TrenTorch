"""
pytest data/app_data/09-vision/03-cnn-architecture/04-full-cnn-classifier/tests.py
"""
...
full_cnn_classifier = load_solution(
    f"09-vision/03-cnn-architecture/{Path(__file__).resolve().parent.name}"
).full_cnn_classifier
stack_cnn_blocks = load_solution("09-vision/03-cnn-architecture/03-stack-multiple-blocks").stack_cnn_blocks
flatten = load_solution("09-vision/03-cnn-architecture/01-flatten").flatten
linear = load_solution("03-classical-ml/01-linear-regression/01-hypothesis-function").linear
softmax = load_solution("03-classical-ml/02-classification/06-softmax-cce").softmax
