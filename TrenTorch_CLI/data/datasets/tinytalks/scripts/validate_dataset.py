...
def validate_line_endings(file_path):
    ...
    if crlf_count > 0:
        ...
        print("     Consider converting to Unix (LF)")
        return False
    else:
...
def validate_content_quality():
    ...
    questions_without_marks = [
        q for q, a in pairs if not (q.endswith("?") or q.endswith("!") or q.endswith("."))
    ]
    if questions_without_marks:
        print(f"  ℹ️  INFO: {len(questions_without_marks)} questions without ending punctuation")
    else:
        print("  ✓ All questions have proper punctuation")
